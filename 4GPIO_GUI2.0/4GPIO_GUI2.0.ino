 ...
 *        Proporciona nivel de iluminancia (0-100%) de sensor ldr
 *        
...
#define FIRMWARE  "4GPIO_GUI_V2.0"
...
const int gpio[]={2,5,18,19,22,23,32}; //GPIos válidos analogRead, ADC1: 32,33,34,35,VP y VN
int valorGPIO[]={LOW,LOW,LOW,LOW,LOW,LOW,LOW};
const int dhtPin = gpio[1];
const int ledPin=gpio[0];
const int ldrPin=gpio[6];
...
const char* password = "fogardixital";
...
// Entorno de s.luminosidad

int ldr=0;
int ldr_tpc=0; //Iluminancia en tanto por ciento

void iniciar_adc(void){
  adcAttachPin(ldrPin);
  analogReadResolution(10); // 12 produce resultados no lineales. recomendable 10 u 11
  analogSetAttenuation(ADC_6db); // default de 11 no recomendable porque es muy ruidoso.
}

void leer_ldr(void ){
  ldr=analogRead(ldrPin);
  ldr_tpc=(ldr*100/1023);
}

void print_ldr(void){
 Serial.println("--------------");
 Serial.print("Ilumninancia :");
 Serial.print(ldr_tpc);
 Serial.println(" %");
 Serial.println("\n");
}
...
void gui_ldr(void){
  cliente.println("<table style='width:100px;'>");
  cliente.println("<tbody>");
  cliente.println("<tr>");
  cliente.print("<td style='width: 60px;'><b>Iluminancia(%):</b></td>");
  cliente.println("</td>");
  cliente.print("<td style='width: 40px;'>");
  cliente.print(ldr_tpc);
  cliente.println("</td>");
  cliente.println("</tr>");
  cliente.println("</tbody>");
  cliente.println("</table>");
...
void loop() {
  ...
  if (cliente) {
    ...
    while (cliente.connected()) {
      ...
      // Cerrando conexión
      delay(100); //necesario para dar tiempo antes de cliente.stop 
      cliente.stop();
     Serial.print("Cliente desconectado de ");
     Serial.println(APsoft);
    Serial.println("");
   }
  }
